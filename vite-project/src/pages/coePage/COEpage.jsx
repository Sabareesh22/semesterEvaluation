...
const COEpage = (props) => {
  ...
  useEffect(() => {
    ...
    const fetchOptions = async () => {
      try {
        const [regulationsRes, yearsRes, batchesRes, semestersRes] =
          await Promise.all([
  ...
  const handleSubmit = async () => {
    if (
      year.length != semester.length ||
      year.length != batch.length ||
      batch.length != semester.length
    ) {
        ...
        >
          ...
          <FormControl sx={{ marginRight: 2, flex: 1 }} required>
            <MultiSelect
              ...
              options={yearOptions.map((data) => ({
                value: data.id,
                label: data.year,
              }))}
            />
            {console.log(
              yearOptions.map((data) => ({ value: data.id, label: data.year }))
            )}
          </FormControl>
          <FormControl sx={{ marginRight: 2, flex: 1 }} required>
            <MultiSelect
          ...
          <FormControl sx={{ flex: 1 }} required>
            <MultiSelect
              ...
              options={semesterOptions.map((data) => ({
                value: data.id,
                label: data.semester,
              }))}
            />
