...
const FacultyChangeRequests = (props) => {
  ...
  const handleApprove = async (request) => {
    try {
      ...
      toast.success(
        `Change request for ${request.new_faculty_name} approved successfully!`
  ...
  const handleReject = async () => {
    if (reason.trim() === "") {
      ...
      try {
        ...
        fetchRequests();
        toast.success(
          `Change request for ${selectedRequest.new_faculty_name} rejected successfully! Reason: ${reason}`
        ); // Detailed success message
        setOpenModal(false);
        setReason("");
        // Optionally refresh requests here
            ...
            <Card style={{ width: "100%" }}>
              <CardContent style={{ padding: "16px", position: "relative" }}>
                ...
                <Typography variant="h6" style={{ fontSize: "1.1rem" }}>
                  Change Request: {request.old_faculty_name}{" "}
                  {request.old_faculty_unique_id} to {request.new_faculty_name}{" "}
                  {request.new_faculty_unique_id}
                </Typography>
                <Typography
                  variant="subtitle1"
