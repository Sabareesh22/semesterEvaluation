...
export function AppRoutes(props) {
    ...
    const parsedRouteObjects = (routes, isPrivate = false) => {
        return routes.map((route) => ({
            path: route.path,
            element: isPrivate ? (
                <ProtectedRoutes setLoading={setLoading} children={route.element} authorizedRole={route.authorizedRole}>
                </ProtectedRoutes>
            ) : (
                route.element
            ),
    ...
    const layoutWrapper = {
        ...
        element: (
            <ProtectedRoutes setLoading={setLoading} authorizedRole={"faculty"}>
                <Layout setLoading={setLoading} loading={loading} title={title} userDetails={props.userDetails} />
            </ProtectedRoutes>
        ),
